...
public interface SplunkResourceManager extends ResourceManager {
  ...
  /**
   * Returns the HTTP Event Collector (HEC) endpoint that this Splunk server is
   * configured to receive events at.
   *
  ...
  /**
   * Returns the Splunk Http Event Collector (HEC) authentication token used to connect to
   * this Splunk instance's HEC service.
   *
   ...
   * <p>Note: Setting the <code>index</code> field in the Splunk event requires the index
   * already being configured in the Splunk instance. Unless using a static Splunk instance,
   * omit this field from the event.
   *
   ...
   * <p>Note: Setting the <code>index</code> field in the Splunk event requires the index
   * already being configured in the Splunk instance. Unless using a static Splunk instance,
   * omit this field from the events.
   *
  ...
  /**
   * Return a list of Splunk events retrieved from the Splunk server based on the
   * given query.
