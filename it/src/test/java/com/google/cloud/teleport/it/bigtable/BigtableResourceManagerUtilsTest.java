...
public class BigtableResourceManagerUtilsTest {
  ...
  public void testGenerateDefaultClustersShouldWorkWhenAllParametersValid() {
    ...
    assertThat(thisCluster.getClusterId()).matches(TEST_ID + "-\\d{8}-\\d{6}-\\d{6}");
    assertThat(thisCluster.getZone()).isEqualTo(ZONE);
    assertThat(thisCluster.getNumNodes()).isEqualTo(NUM_NODES);
    assertThat(thisCluster.getStorageType()).isEqualTo(STORAGE_TYPE);
  }
  ...
  public void testGenerateDefaultClustersShouldShortenTestIdWhenTooLong() {
    ...
    assertThat(cluster.iterator().next().getClusterId()).matches("longer--\\d{8}-\\d{6}-\\d{6}");
  }
