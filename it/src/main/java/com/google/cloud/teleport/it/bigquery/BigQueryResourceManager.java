...
public interface BigQueryResourceManager {
     ...
     * BigQueryResourceManager#createTable(String, Schema)} to be called for the target table beforehand.
     *
     * @param tableRow A row object representing the table row.
     * @throws BigQueryResourceManagerException if method is called after resources have been cleaned up, if
     *     the manager object has no dataset, if the table does not exist or if there is an Exception when
    ...
    /**
     * Reads all the rows in a table. This method requires {@link BigQueryResourceManager#createTable(String, Schema)}
     * to be called for the target table beforehand.
     *
    ...
    /**
     * Deletes all created resources (dataset and tables) and cleans up the BigQuery
